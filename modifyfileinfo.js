...
const observer = new MutationObserver((mutations, observer) => {
  ...
  for (var recordno in mutations) {
    ...
    try {
      if (target.includes('chevron-down') && source == 'File Info') {
    ...
    try {
      if (target.includes('chevron-up') && source == 'File Info') {
        //console.log(mutations[recordno])
    ...
    let chevvy
    try {
      chevvy = document.querySelector("#info_panel > div > div > div > div:nth-child(2) > div:nth-child(1) > div > button > div").lastChild.className
    } catch (err) { chevvy = 'ok'; }
...
const memserver = new MutationObserver(function (mutations, mutationInstance) {
  ...
  try {
    if (someDiv.innerText.includes('File Info')) {
